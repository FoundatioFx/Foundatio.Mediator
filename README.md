...
## ⚙️ Generator Configuration Options

Foundatio.Mediator provides several MSBuild properties to customize code generation behavior. Add these to your project file's `<PropertyGroup>`:

```xml
<PropertyGroup>
    <!-- Handler lifetime management (None/Singleton/Scoped/Transient) -->
    <MediatorHandlerLifetime>None</MediatorHandlerLifetime>

    <!-- Disable C# interceptors (falls back to DI lookup) -->
...
|----------|---------|-------------|
| `MediatorHandlerLifetime` | `None` | Controls handler DI registration. Options: `None` (not registered), `Singleton`, `Scoped`, `Transient` |
| `MediatorDisableInterceptors` | `false` | When `true`, disables C# interceptors and uses DI lookup for all calls |
| `MediatorDisableOpenTelemetry` | `false` | When `true`, removes OpenTelemetry instrumentation from generated code |
...
## 👁️ Viewing Generated Source Code

To see the source code generated by Foundatio.Mediator, add the following to your project file:
...
After rebuilding your project, the generated files will appear in the `Generated` folder. This is helpful for:

- **Understanding interceptor logic** - See exactly how your mediator calls are transformed
- **Debugging performance** - Verify the generated code matches your expectations
- **Learning the framework** - Understand how handlers and middleware are wired together
- **Troubleshooting issues** - Examine generated code when encountering problems

The generated files include:

- Handler wrapper methods for interceptors
- DI registration modules
- Middleware pipeline implementations
