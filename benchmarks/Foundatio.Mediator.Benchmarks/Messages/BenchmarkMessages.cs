...
public record PingCommand(string Id) : IRequest, MediatorLib.ICommand;

// Scenario 2: Query with return value for InvokeAsync<T>
public record GetOrder(int Id) : IRequest<Order>, MediatorLib.IQuery<Order>;

// Scenario 4: FullQuery - Query with dependency injection
public record GetFullQuery(int Id) : IRequest<Order>, MediatorLib.IQuery<Order>;
...
public record CreateOrder(int CustomerId, decimal Amount) : IRequest<Order>, MediatorLib.IRequest<Order>;
...
public record GetCachedOrder(int Id) : IRequest<Order>, MediatorLib.IQuery<Order>;
