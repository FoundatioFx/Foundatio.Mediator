...
- Foundatio notifications are best effort: publishing writes to a bounded drop-oldest buffer, and the outbound worker publishes to SNS sequentially. MassTransit bus publication awaits transport acceptance and supports durable consumer/error handling. These are different delivery guarantees. Enlarging Foundatio's buffer in the lossless burst matrix retains the offered workload; it does not change the guarantee or add backpressure.
- Foundatio's notification host subscribes to its own topic and filters its own messages after receipt. That infrastructure/traffic is included. MassTransit's temporary bus endpoint is included. Every business queue/topic and temporary bus queue gets an explicit run prefix.
