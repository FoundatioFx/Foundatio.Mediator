...
The [distributed benchmark suite](https://github.com/FoundatioFx/Foundatio.Mediator/tree/main/benchmarks/Foundatio.Mediator.Distributed.Benchmarks) compares local mediator dispatch, in-memory distributed workers, and SQS/SNS with MassTransit on the same transport. It runs broker consumers in separate processes, verifies completed deliveries, reports API and delivery latency separately, and includes fan-out, fixed-rate load, and notification-buffer saturation scenarios. Use Floci for repeatable local experiments and the AWS configuration for measurements against the actual service; emulator throughput is not AWS capacity.
