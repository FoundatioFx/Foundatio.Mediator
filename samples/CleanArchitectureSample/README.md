...
| **`[QueueLock]` single-flight** | `GenerateBankFileHandler`, native `Foundatio.Lock.RedisLockProvider`, "Enqueue 2 bank files" control |
...
│   ├── Program.cs                         # native infrastructure and Mediator handler registration
...
{
    public async Task<Result> HandleAsync(DemoExportJob message, MessageProcessingContext context, TenantContext tenant, IMediator mediator, CancellationToken ct)
    {
        for (int i = 1; i <= steps; i++)
        {
            ct.ThrowIfCancellationRequested();
            await Task.Delay(message.StepDelayMs, ct);
            await context.ReportProgressAsync(percent, $"Step {i} of {steps} on {host.HostId}", ct);
        }

        await mediator.PublishAsync(new DemoJobCompleted(context.JobId!, context.QueueName, host.HostId, tenant.TenantId), ct);
        return Result.Ok();
    }
}
```
...
public class FlakyWebhookHandler(...)
{
    public async Task<Result> HandleAsync(DeliverWebhook message, MessageProcessingContext context, IMediator mediator, CancellationToken ct)
    {
        ...
        if (!context.Headers.ContainsKey(MessageHeaders.OriginalJobId) && context.DequeueCount <= message.FailTimes)
            return Result.Error($"{uri.Host} returned 503 on attempt {context.DequeueCount}");   // retried

        await mediator.PublishAsync(new WebhookDelivered(message.Url, context.DequeueCount, host.HostId, context.JobId!, context.QueueName), ct);
        return Result.Ok();
    }
}
```

`RetryDelays` selects the schedule: attempt 1 fails → wait 1 s → attempt 2 fails → wait 3 s → attempt 3 fails → dead letter. The message carries the reason, attempt count, original queue, correlation ID, and tracked job ID. `MessageProcessingContext.DequeueCount` is the attempt number; `MaxAttempts` and received `Headers` are also available on the context.
...
**UI:** choose **3 · move to dead letters**, then **Enqueue webhook**. After its retries, follow **View queue**, then **Dead letters**, or click the red queue count on the dashboard. **Inspect DeliverWebhook** expands the payload preview and all headers; **Original job** opens the retained failure. **Retry message** replays one message; **Retry available** replays up to the selected batch limit. Both remove successfully requeued messages from dead letters and return new receipt links. The receipt follows up to ten new jobs through completion or failure, and stops polling terminal jobs. A retry that fails again is labeled **Failed again after retry**, with links to its failed job and the previous job; it is a new dead-letter record. In this simulated webhook handler, replay models the operator fixing the remote service: the replay marker lets the new job succeed. No external HTTP request is made. The original job remains Failed. Replaying the deliberately unrecoverable export continues to fail, demonstrating that replay alone does not repair a payload.
...
Replay preserves the original failure, creates new tracked job identities, and records `message.execution.original_id`. Failures arriving during the replay run are left for another decision. Replay is at-least-once: a partial transport failure may leave an Enqueue unknown job; reconcile before retrying the operation.
...
- **Dead letters** supports payload/header inspection, per-record **Retry message** and **Purge message** actions, and bulk retry/flush. Row actions remain visible but disabled without administrator access; a sign-in link returns to the same queue and view. Individual purge confirms the selected message ID and leaves other records available; all purge/flush operations retain failed-job history. It inspects on entry or explicit refresh, never on the monitoring timer.
...
Transport counts are approximate, while tracked job state is shared in Redis. Missing transport statistics display as unknown, and failed refreshes preserve previous data with a stale-data warning. If shared counter history is unavailable, counters are explicitly labeled as belonging only to the responding process. Svelte 5 runes and keyed rows update the dashboard, job lists, and charts in place. Background polls keep controls enabled and preserve focus, expanded payloads, chart selection, and the last loaded data. Explicit refreshes and view changes coalesce behind an in-flight read; dead-letter actions wait for inspection leases to be released. **Pause updates** stops the 2.5-second monitoring poll; **Refresh now** still works. Live events are a separate best-effort authenticated subscription and are not the source of job status. Existing `/queues?queue=...` links redirect to the dedicated page, and signing in from queue details returns to the same view.
...
Queue labels are separate from routing identities. For example, setting `DisplayName = "Bank file generation"` on the bank handler's `[Queue]` attribute labels `sample-GenerateBankFile` without renaming it. Audit and notification handlers put `[Queue(DisplayName = "...", Group = "events")]` on each handler method so labels live next to the subscriptions they describe. The API retains one `QueueOverrides` example to label the shared `order-created` queue at startup. The dashboard shows both names and searches either; queue details use the display name in their heading and show both values in Settings. Unnamed queues retain their physical name as the label. Changing a label preserves URLs, messages, dead letters, worker selection, and job history.
...
`ObservabilityMiddleware` passes the `Stopwatch` returned from `Before` into `After` and `Finally`, and takes an optional `MessageProcessingContext` parameter to log whether a message came from HTTP, the bus, or a queue. `ValidationMiddleware` short-circuits with `Result.Invalid(...)` before the handler runs. Ordering uses `OrderBefore`/`OrderAfter` instead of numbers:
