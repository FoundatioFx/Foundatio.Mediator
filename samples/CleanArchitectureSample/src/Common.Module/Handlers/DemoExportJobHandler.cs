...
public class DemoExportJobHandler(HostInfo host, ILogger<DemoExportJobHandler> logger)
{
    public async Task<Result> HandleAsync(DemoExportJob message, MessageProcessingContext context, TenantContext tenant, IMediator mediator, CancellationToken ct)
    {
        ...
        logger.LogInformation("Starting export job {JobId} for {Tenant} on {HostId} ({Steps} steps)",
            context.JobId, tenant, host.HostId, steps);
        ...
        for (int i = 1; i <= steps; i++)
        {
            ...
            if (i == Math.Min(2, steps) && context.DequeueCount <= message.FailTimes)
                return Result.Error($"Simulated transient failure on attempt {context.DequeueCount}; retry starts from the beginning.");

            int percent = (int)((double)i / steps * 100);
            await context.ReportProgressAsync(percent, $"Step {i} of {steps} on {host.HostId}", ct).ConfigureAwait(false);
        }

        logger.LogInformation("Export job {JobId} completed on {HostId}", context.JobId, host.HostId);
