...
public class ImportProductCatalogHandler(HostInfo host, ILogger<ImportProductCatalogHandler> logger)
{
    ...
    public async Task<Result> HandleAsync(ImportProductCatalog message, QueueContext queueContext, TenantContext tenant, IMediator mediator, CancellationToken ct)
    {
        ...
        logger.LogInformation("Starting catalog import {JobId} for {Tenant} on {HostId} ({Rows} rows)",
            queueContext.JobId, tenant, host.HostId, rows);
        ...
        for (int row = 1; row <= rows; row++)
        {
            ...
            if (row % reportEvery == 0 || row == rows)
                await queueContext.ReportProgressAsync(row * 100 / rows, $"Imported {row} of {rows} rows on {host.HostId}", ct).ConfigureAwait(false);
        }

        logger.LogInformation("Catalog import {JobId} completed on {HostId}", queueContext.JobId, host.HostId);
