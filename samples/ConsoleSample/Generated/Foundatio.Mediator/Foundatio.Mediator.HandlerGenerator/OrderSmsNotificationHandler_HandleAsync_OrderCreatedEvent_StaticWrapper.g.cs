...
{
    internal static class OrderSmsNotificationHandler_HandleAsync_OrderCreatedEvent_StaticWrapper
    {
...
#pragma warning restore CS0219
            ...
            try
            {
                ...
                await handlerInstance.HandleAsync(message, serviceProvider.GetRequiredService<ConsoleSample.Services.SmsNotificationService>(), cancellationToken);
        ...
        private static ConsoleSample.Handlers.OrderSmsNotificationHandler? _handler;
        private static readonly object _lock = new object();

        private static ConsoleSample.Handlers.OrderSmsNotificationHandler GetOrCreateHandler(IServiceProvider serviceProvider)
        {
            if (_handler != null)
                return _handler;

            var handlerFromDI = serviceProvider.GetService<ConsoleSample.Handlers.OrderSmsNotificationHandler>();
            ...
            lock (_lock)
            {
                ...
                _handler = ActivatorUtilities.CreateInstance<ConsoleSample.Handlers.OrderSmsNotificationHandler>(serviceProvider);
                return _handler;
            }
        }
