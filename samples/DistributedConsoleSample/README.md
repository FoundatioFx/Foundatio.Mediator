...
No Docker is needed. The sample configures native Foundatio in-memory messaging and execution tracking, starts a host, rejects an invalid request before sending, enqueues a greeting, reports progress through `MessageProcessingContext`, and observes completion through `IMessageExecutionStore`.
