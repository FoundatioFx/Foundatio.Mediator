...
public class RedisJobStateStoreOptions
{
    ...
    /// </summary>
    /// <remarks>
    /// The queue worker passes <c>DistributedQueueOptions.JobStateExpiry</c> on every write, so this only
    /// takes effect for direct callers of the store. Writes that leave a job <see cref="QueueJobStatus.Queued"/>
    /// or <see cref="QueueJobStatus.Processing"/> are raised to at least <see cref="NonTerminalExpiry"/>.
    /// </remarks>
    public TimeSpan? DefaultExpiry { get; set; } = TimeSpan.FromHours(24);

    /// <summary>
    /// Minimum TTL for a job while it is <see cref="QueueJobStatus.Queued"/> or <see cref="QueueJobStatus.Processing"/>,
