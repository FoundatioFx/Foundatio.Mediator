...
{
    /// <summary>
    /// Tracks per-file generation counts. Key is the hint name (file path).
    /// This persists across generator runs within the same compiler process.
    /// </summary>
    private static readonly ConcurrentDictionary<string, int> _fileGenerationCounts = new();

    public static void AddGeneratedFileHeader(this IndentedStringBuilder source, bool includeGenerationCounter = false, string? hintName = null)
    {
        var semanticVersion = GetSemanticVersion();

        source.AppendLine("// <auto-generated>");
        source.AppendLine($"// Generated by Foundatio.Mediator v{semanticVersion}");
        if (includeGenerationCounter && !string.IsNullOrEmpty(hintName))
        {
            var count = _fileGenerationCounts.AddOrUpdate(hintName!, 1, (_, existing) => existing + 1);
            source.AppendLine($"// Generation #{count} at {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.fff} UTC");
        }
