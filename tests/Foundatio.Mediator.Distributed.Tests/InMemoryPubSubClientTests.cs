...
public class InMemoryPubSubClientTests(ITestOutputHelper output) : TestWithLoggingBase(output)
{
    ...
    public async Task PublishAsync_WithNoSubscribers_DoesNotThrow()
    {
        ...
        await bus.PublishAsync("test-topic", "hello"u8.ToArray(), cancellationToken: TestCancellationToken);
    }
    ...
    public async Task SubscribeAsync_ReceivesPublishedMessage()
    {
        ...
        await bus.PublishAsync("test-topic", "hello"u8.ToArray(), headers, TestCancellationToken);
    ...
    public async Task SubscribeAsync_MultipleSubscribers_AllReceive()
    {
        ...
        await bus.PublishAsync("topic", "data"u8.ToArray(), cancellationToken: TestCancellationToken);
    ...
    public async Task SubscribeAsync_DifferentTopics_OnlyMatchingReceives()
    {
        ...
        await bus.PublishAsync("topic-a", "only-a"u8.ToArray(), cancellationToken: TestCancellationToken);
    ...
    public async Task DisposeSubscription_StopsReceiving()
    {
        ...
        await bus.PublishAsync("topic", "msg1"u8.ToArray(), cancellationToken: TestCancellationToken);
        Assert.True(await signal.WaitAsync(TimeSpan.FromSeconds(5)));
        ...
        await bus.PublishAsync("topic", "msg2"u8.ToArray(), cancellationToken: TestCancellationToken);
        await Task.Delay(200, TestCancellationToken);
    ...
    public async Task PublishAsync_HeadersAreReadOnly()
    {
        ...
        await bus.PublishAsync("topic", "test"u8.ToArray(), new Dictionary<string, string>
        {
            ["h1"] = "v1", ["h2"] = "v2"
        }, TestCancellationToken);
