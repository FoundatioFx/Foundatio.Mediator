...
{
    internal static class BaseMessageHandler_HandleAsync_BaseMessage_StaticWrapper
    {
        public static async System.Threading.Tasks.Task HandleAsync(Foundatio.Mediator.Tests.BaseMessage message, IServiceProvider serviceProvider, CancellationToken cancellationToken)
        {
        ...
        {
            var typedMessage = (Foundatio.Mediator.Tests.BaseMessage)message;
            var serviceProvider = ((Mediator)mediator).ServiceProvider;
            await HandleAsync(typedMessage, serviceProvider, cancellationToken);
            return new object();
        }

        private static Foundatio.Mediator.Tests.BaseMessageHandler? _handler;
        private static readonly object _lock = new object();

        private static Foundatio.Mediator.Tests.BaseMessageHandler GetOrCreateHandler(IServiceProvider serviceProvider)
        {
            if (_handler != null)
                return _handler;

            var handlerFromDI = serviceProvider.GetService<Foundatio.Mediator.Tests.BaseMessageHandler>();
            ...
            lock (_lock)
            {
                ...
                _handler = ActivatorUtilities.CreateInstance<Foundatio.Mediator.Tests.BaseMessageHandler>(serviceProvider);
                return _handler;
            }
        }
