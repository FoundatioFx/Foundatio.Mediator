...
{
    internal static class CallSiteTestNotification2Handler_HandleAsync_CallSiteTestNotification_StaticWrapper
    {
        public static async System.Threading.Tasks.Task HandleAsync(Foundatio.Mediator.Tests.CallSiteTestNotification message, IServiceProvider serviceProvider, CancellationToken cancellationToken)
        {
            var handlerInstance = GetOrCreateHandler(serviceProvider);
            await handlerInstance.HandleAsync(message, serviceProvider.GetRequiredService<Foundatio.Mediator.Tests.CallSiteTestService>(), cancellationToken);
        }
        ...
        {
            var typedMessage = (Foundatio.Mediator.Tests.CallSiteTestNotification)message;
            var serviceProvider = ((Mediator)mediator).ServiceProvider;
            await HandleAsync(typedMessage, serviceProvider, cancellationToken);
            return new object();
        }

        private static Foundatio.Mediator.Tests.CallSiteTestNotification2Handler? _handler;
        private static readonly object _lock = new object();

        private static Foundatio.Mediator.Tests.CallSiteTestNotification2Handler GetOrCreateHandler(IServiceProvider serviceProvider)
        {
            if (_handler != null)
                return _handler;

            var handlerFromDI = serviceProvider.GetService<Foundatio.Mediator.Tests.CallSiteTestNotification2Handler>();
            ...
            lock (_lock)
            {
                ...
                _handler = ActivatorUtilities.CreateInstance<Foundatio.Mediator.Tests.CallSiteTestNotification2Handler>(serviceProvider);
                return _handler;
            }
        }
