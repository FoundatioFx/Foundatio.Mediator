...
{
    internal static class DomainEventAuditorHandler_HandleAsync_DomainEvent_StaticWrapper
    {
        public static async System.Threading.Tasks.Task HandleAsync(Foundatio.Mediator.Tests.PolymorphicHandlingTest.DomainEvent message, IServiceProvider serviceProvider, CancellationToken cancellationToken)
        {
        ...
        {
            var typedMessage = (Foundatio.Mediator.Tests.PolymorphicHandlingTest.DomainEvent)message;
            var serviceProvider = ((Mediator)mediator).ServiceProvider;
            await HandleAsync(typedMessage, serviceProvider, cancellationToken);
            return new object();
        }

        private static Foundatio.Mediator.Tests.PolymorphicHandlingTest.DomainEventAuditorHandler? _handler;
        private static readonly object _lock = new object();

        private static Foundatio.Mediator.Tests.PolymorphicHandlingTest.DomainEventAuditorHandler GetOrCreateHandler(IServiceProvider serviceProvider)
        {
            if (_handler != null)
                return _handler;

            var handlerFromDI = serviceProvider.GetService<Foundatio.Mediator.Tests.PolymorphicHandlingTest.DomainEventAuditorHandler>();
            ...
            lock (_lock)
            {
                ...
                _handler = ActivatorUtilities.CreateInstance<Foundatio.Mediator.Tests.PolymorphicHandlingTest.DomainEventAuditorHandler>(serviceProvider);
                return _handler;
            }
        }
