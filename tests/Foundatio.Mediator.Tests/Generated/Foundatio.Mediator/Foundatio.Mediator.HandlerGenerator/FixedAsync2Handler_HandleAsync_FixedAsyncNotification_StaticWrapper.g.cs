...
{
    internal static class FixedAsync2Handler_HandleAsync_FixedAsyncNotification_StaticWrapper
    {
        public static async System.Threading.Tasks.Task HandleAsync(Foundatio.Mediator.Tests.FixedAsyncNotification message, IServiceProvider serviceProvider, CancellationToken cancellationToken)
        {
            var handlerInstance = GetOrCreateHandler(serviceProvider);
            await handlerInstance.HandleAsync(message, serviceProvider.GetRequiredService<Foundatio.Mediator.Tests.FixedTestService>(), cancellationToken);
        }
        ...
        {
            var typedMessage = (Foundatio.Mediator.Tests.FixedAsyncNotification)message;
            var serviceProvider = ((Mediator)mediator).ServiceProvider;
            await HandleAsync(typedMessage, serviceProvider, cancellationToken);
            return new object();
        }

        private static Foundatio.Mediator.Tests.FixedAsync2Handler? _handler;
        private static readonly object _lock = new object();

        private static Foundatio.Mediator.Tests.FixedAsync2Handler GetOrCreateHandler(IServiceProvider serviceProvider)
        {
            if (_handler != null)
                return _handler;

            var handlerFromDI = serviceProvider.GetService<Foundatio.Mediator.Tests.FixedAsync2Handler>();
            ...
            lock (_lock)
            {
                ...
                _handler = ActivatorUtilities.CreateInstance<Foundatio.Mediator.Tests.FixedAsync2Handler>(serviceProvider);
                return _handler;
            }
        }
