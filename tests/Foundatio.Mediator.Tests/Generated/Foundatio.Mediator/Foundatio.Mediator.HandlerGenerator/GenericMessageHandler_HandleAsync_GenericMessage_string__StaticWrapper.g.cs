...
{
    internal static class GenericMessageHandler_HandleAsync_GenericMessage_string__StaticWrapper
    {
        public static async System.Threading.Tasks.Task HandleAsync(Foundatio.Mediator.Tests.GenericMessage<string> message, IServiceProvider serviceProvider, CancellationToken cancellationToken)
        {
        ...
        {
            var typedMessage = (Foundatio.Mediator.Tests.GenericMessage<string>)message;
            var serviceProvider = ((Mediator)mediator).ServiceProvider;
            await HandleAsync(typedMessage, serviceProvider, cancellationToken);
            return new object();
        }

        private static Foundatio.Mediator.Tests.GenericMessageHandler? _handler;
        private static readonly object _lock = new object();

        private static Foundatio.Mediator.Tests.GenericMessageHandler GetOrCreateHandler(IServiceProvider serviceProvider)
        {
            if (_handler != null)
                return _handler;

            var handlerFromDI = serviceProvider.GetService<Foundatio.Mediator.Tests.GenericMessageHandler>();
            ...
            lock (_lock)
            {
                ...
                _handler = ActivatorUtilities.CreateInstance<Foundatio.Mediator.Tests.GenericMessageHandler>(serviceProvider);
                return _handler;
            }
        }
