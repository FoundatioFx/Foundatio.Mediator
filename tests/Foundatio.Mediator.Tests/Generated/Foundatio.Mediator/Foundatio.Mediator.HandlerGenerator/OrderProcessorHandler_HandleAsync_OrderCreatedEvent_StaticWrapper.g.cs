...
{
    internal static class OrderProcessorHandler_HandleAsync_OrderCreatedEvent_StaticWrapper
    {
        public static async System.Threading.Tasks.Task HandleAsync(Foundatio.Mediator.Tests.PolymorphicHandlingTest.OrderCreatedEvent message, IServiceProvider serviceProvider, CancellationToken cancellationToken)
        {
        ...
        {
            var typedMessage = (Foundatio.Mediator.Tests.PolymorphicHandlingTest.OrderCreatedEvent)message;
            var serviceProvider = ((Mediator)mediator).ServiceProvider;
            await HandleAsync(typedMessage, serviceProvider, cancellationToken);
            return new object();
        }

        private static Foundatio.Mediator.Tests.PolymorphicHandlingTest.OrderProcessorHandler? _handler;
        private static readonly object _lock = new object();

        private static Foundatio.Mediator.Tests.PolymorphicHandlingTest.OrderProcessorHandler GetOrCreateHandler(IServiceProvider serviceProvider)
        {
            if (_handler != null)
                return _handler;

            var handlerFromDI = serviceProvider.GetService<Foundatio.Mediator.Tests.PolymorphicHandlingTest.OrderProcessorHandler>();
            ...
            lock (_lock)
            {
                ...
                _handler = ActivatorUtilities.CreateInstance<Foundatio.Mediator.Tests.PolymorphicHandlingTest.OrderProcessorHandler>(serviceProvider);
                return _handler;
            }
        }
