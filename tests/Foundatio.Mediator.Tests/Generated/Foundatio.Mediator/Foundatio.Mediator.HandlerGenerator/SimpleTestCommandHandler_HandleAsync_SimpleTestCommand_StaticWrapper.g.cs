...
{
    internal static class SimpleTestCommandHandler_HandleAsync_SimpleTestCommand_StaticWrapper
    {
        public static async System.Threading.Tasks.Task HandleAsync(Foundatio.Mediator.Tests.SimpleTestCommand message, IServiceProvider serviceProvider, CancellationToken cancellationToken)
        {
        ...
        {
            var typedMessage = (Foundatio.Mediator.Tests.SimpleTestCommand)message;
            var serviceProvider = ((Mediator)mediator).ServiceProvider;
            await HandleAsync(typedMessage, serviceProvider, cancellationToken);
            return new object();
        }

        [global::System.Runtime.CompilerServices.InterceptsLocation(1, "fu4id1aiSBQYcG2xaNqrp+oHAABEZXBlbmRlbmN5SW5qZWN0aW9uVGVzdHMuY3M=")] // C:\Users\eric\Projects\Foundatio\Foundatio.Mediator\tests\Foundatio.Mediator.Tests\DependencyInjectionTests.cs(64,25)
        ...
        private static Foundatio.Mediator.Tests.SimpleTestCommandHandler? _handler;
        private static readonly object _lock = new object();

        private static Foundatio.Mediator.Tests.SimpleTestCommandHandler GetOrCreateHandler(IServiceProvider serviceProvider)
        {
            if (_handler != null)
                return _handler;

            var handlerFromDI = serviceProvider.GetService<Foundatio.Mediator.Tests.SimpleTestCommandHandler>();
            ...
            lock (_lock)
            {
                ...
                _handler = ActivatorUtilities.CreateInstance<Foundatio.Mediator.Tests.SimpleTestCommandHandler>(serviceProvider);
                return _handler;
            }
        }
